...
var leftSelectButtonId = 'btnLeftBoxButtonSel';
var calValueId = "hdnCalendarValue";
var dateValueId = "hdnDateValue";
...
function carenda(num, format, selectymd) {
    var now = new Date();
    var year;
    var month;
    var date;
    var dValue = document.getElementById(dateValueId);
    document.getElementById(leftSelectButtonId).disabled = true;

    if (!format) {
        var fromatObj = document.getElementById('hdnPageCommonDateFormat');
        format = "yyyy/MM/dd"
        if (fromatObj !== null) {
            if (fromatObj.value !== "") {
                format = fromatObj.value;
            }
        }
    }

    // 第3引数が未指定の場合
    if (selectymd === undefined) {
        selectymd = '1900/01/01';
        //selectymd = '2018/10/25';
    }
    var selymd = new Date(selectymd);
    // 第三引数日付をフォーマットに基づいて整形
    var selectymdDtm = new Date(selectymd);
    var selYear;
    var selMonth;
    var selDate;
    if (selectymdDtm !== "Invalid Date") {
        selYear = selectymdDtm.getFullYear();
        selMonth = selectymdDtm.getMonth() + 1;
        selDate = selectymdDtm.getDate();
    } else {
    ...
    selectymd = format
        .replace(/([Yy]+)/, selYear)
        .replace(/([Mm]+)/, ("00" + selMonth).slice(-2))
        .replace(/([Dd]+)/, ("00" + selDate).slice(-2));
    // memo１日＝６０秒×６０分×２４時間＝８６４００秒、８６４００秒＝８６４０００００ミリ秒 
    switch (parseInt(num)) {
        case 0:
            //初期表示
            if (document.getElementById(calValueId).value === "") {
                year = now.getFullYear();
                month = now.getMonth() + 1;
                date = now.getDate();
            } else {
                var ymd = new Date(document.getElementById(calValueId).value);
                if (ymd !== "Invalid Date") {
        ...
        case 1:
            //前月表示
            var backMDate = new Date(parseInt(dValue.value) - 24 * 60 * 60 * 1000 * 1);
        ...
        case 2:
            //翌月表示
            var nextMDate = new Date(parseInt(dValue.value) + 24 * 60 * 60 * 1000 * 31);
    ...
    dValue.value = (new Date(year, month - 1, 1)).getTime();
    ...
    //editMsg = "";
    //editMsg += "<table border=0 cellspacing=3><tr><td style='cursor:pointer;background:white' align='center' onclick='carenda(1)'><b style='font-size:large;'>&lt;&lt;</b></td><td colspan='5' align='center' style='background:white'><b style='font-size:x-large'>" + monthEng[month] + " " + year + "</b></td><td style='cursor:pointer;background:white' align='center' onclick='carenda(2)'><b style='font-size:large'>&gt;&gt;</b></td></tr>\n";
    //editMsg += "<tr>" + defTD("Sun.", "red", kabe_sun) + defTD("Mon.", "black", kabe_mon) + defTD("Tue.", "black", kabe_tue) + defTD("Web.", "black", kabe_wed) + defTD("Thu.", "black", kabe_thu) + defTD("Fri.", "black", kabe_fri) + defTD("Sat.", "blue", kabe_sat) + "</tr>\n";
    //editMsg += "<tr>";

    var dateTypeObj = new Date(year, month - 1, 1);
    var locale = "en-US"
    var options = { year: "numeric", month: "short" };	// オプションのパラメーター day や weekdayを設定するとほかの情報も出ます。
    editMsg = "";
    //editMsg += "<table border=0 cellspacing=3><tr><td style='cursor:pointer;background:white' align='center' onclick='carenda(1)'><b style='font-size:large;'>&lt;&lt;</b></td><td colspan='5' align='center' style='background:white'><b style='font-size:x-large'>" + dateTypeObj.toLocaleString(locale, options) + "</b></td><td style='cursor:pointer;background:white' align='center' onclick='carenda(2)'><b style='font-size:large'>&gt;&gt;</b></td></tr>\n";
    if ((year > selymd.getFullYear()) || (year >= selymd.getFullYear() && month > selymd.getMonth() + 1)) {
        editMsg += "<table border=0 cellspacing=3><tr><td style='cursor:pointer;background:white' align='center' onclick='carenda(1,\"" + format + "\",\"" + selectymd + "\");'><b style='font-size:large;'>&lt;&lt;</b></td><td colspan='5' align='center' style='background:white'><b style='font-size:x-large'>" + dateTypeObj.toLocaleString(locale, options) + "</b></td><td style='cursor:pointer;background:white' align='center' onclick='carenda(2,\"" + format + "\",\"" + selectymd + "\");'><b style='font-size:large'>&gt;&gt;</b></td></tr>\n";
    } else {
        editMsg += "<table border=0 cellspacing=3><tr><td style='cursor:pointer;background:white' align='center'></td><td colspan='5' align='center' style='background:white'><b style='font-size:x-large'>" + dateTypeObj.toLocaleString(locale, options) + "</b></td><td style='cursor:pointer;background:white' align='center' onclick='carenda(2,\"" + format + "\",\"" + selectymd + "\");'><b style='font-size:large'>&gt;&gt;</b></td></tr>\n";
    }
    editMsg += "<tr>" + defTD("Sun.", "red", kabe_sun) + defTD("Mon.", "black", kabe_mon) + defTD("Tue.", "black", kabe_tue) + defTD("Web.", "black", kabe_wed) + defTD("Thu.", "black", kabe_thu) + defTD("Fri.", "black", kabe_fri) + defTD("Sat.", "blue", kabe_sat) + "</tr>\n";
    editMsg += "<tr>";
    ...
    if (date !== -1) {
        ...
        //firstAltYMD = year + "/" + ("00" + month).slice(-2) + "/" + ("00" + date).slice(-2);
        firstAltYMD = format
            .replace(/([Yy]+)/, year)
    ...
    for (i = 1; i <= last_date[month - 1]; i++) {
        ...
        var kabeColor;
        var fontColor;
        //var altYMD = year + "/" + ("00" + month).slice(-2) + "/" + ("00" + i).slice(-2);
        var altYMD = format
            .replace(/([Yy]+)/, year)
            .replace(/([Mm]+)/, ("00" + month).slice(-2))
            .replace(/([Dd]+)/, ("00" + i).slice(-2));
        var altMsg = "&nbsp";
        //曜日別基本設定
        switch (dayIndex) {
        ...
        //選択された日（オレンジに）
        var ymd2 = new Date(document.getElementById(calValueId).value);
        ...
        editMsg += defTD2(i, fontColor, kabeColor, altYMD, altMsg, selectymd);
...
function defTD(str, iro, kabe) {
    ...
    return "<td style='cursor:default;width:45px;hight:30px;background:" + kabe + "; color:" + iro + ";' align='center' ><b style='font-size: large;'>" + str + "</b></td>";
}
function defTD2(str, iro, kabe, altYMD, altMsg, selectymd) {
    var editHTML = "";
    //editHTML += "<td id='" + altYMD + "' style='cursor:default;width:35px;hight:30px;background:" + kabe + "; color:" + iro + ";' align='center' ";
    editHTML += "<td id='" + altYMD + "' style='cursor:default;width:45px;hight:30px;background:" + kabe + "; color:" + iro + ";' align='center' ";
...
function setDate(altYMD) {
    document.getElementById(calValueId).value = altYMD;
    document.getElementById(leftSelectButtonId).disabled = false;
    document.getElementById(leftSelectButtonId).click();
}
