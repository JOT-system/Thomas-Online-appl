...
''' ユーザーマスタ検索画面クラス
''' </summary>
Public Class COM00005SELECT
    Inherits GbPageCommon
    Private Const CONST_MAPID As String = "COM00005S"   '自身のMAPID
    Private Const CONST_BASEID As String = "COM00005"
        ...
        If activeViewObj IsNot Nothing Then
            Select Case activeViewObj.ID
                ...
                Case Me.vLeftOrg.ID 'アクティブなビューが組織コード
                    ...
                    If targetObject IsNot Nothing Then
                        Dim txtobj As TextBox = DirectCast(targetObject, TextBox)
                        If Me.lbOrg.SelectedItem IsNot Nothing Then
                            txtobj.Text = Me.lbOrg.SelectedItem.Value
                            Dim parts As String()
                            parts = Split(Me.lbOrg.SelectedItem.Text, ":", -1, CompareMethod.Text)
                            Me.lblOrgText.Text = parts(1)
                            txtobj.Focus()
                        Else
                            txtobj.Text = ""
                            Me.lblOrgText.Text = ""
    ...
    Private Sub LangSetting(ByVal lang As String)
        ...
        AddLangSetting(dicDisplayText, Me.lblOrg, "組織コード", "Organization Code")
    ...
    Public Sub DefaultValueSet()
        ...
        Else
            ...
            Dim prevSelectProductPage As COM00005USER = DirectCast(Page.PreviousPage, COM00005USER)

            '有効年月日
            txtStYMD.Text = FormatDateContrySettings(DirectCast(prevSelectProductPage.FindControl("hdnSelectedStYMD"), HiddenField).Value, Convert.ToString(HttpContext.Current.Session("DateFormat")))
            txtEndYMD.Text = FormatDateContrySettings(DirectCast(prevSelectProductPage.FindControl("hdnSelectedEndYMD"), HiddenField).Value, Convert.ToString(HttpContext.Current.Session("DateFormat")))
            '会社コード　
            txtCompany.Text = DirectCast(prevSelectProductPage.FindControl("hdnSelectedCompCode"), HiddenField).Value
            txtCompany_Change()
            '組織コード　
            txtOrg.Text = DirectCast(prevSelectProductPage.FindControl("hdnSelectedOrgCode"), HiddenField).Value
            txtOrg_Change()
    ...
    Public Sub variableSet()
        ...
        If COA0016VARIget.ERR = C_MESSAGENO.NORMAL Then
            txtStYMD.Text = FormatDateContrySettings(COA0016VARIget.VALUE, Convert.ToString(HttpContext.Current.Session("DateFormat")))
        Else
        ...
        COA0016VARIget.COA0016VARIget()
        If COA0016VARIget.ERR = C_MESSAGENO.NORMAL Then
            txtEndYMD.Text = FormatDateContrySettings(COA0016VARIget.VALUE, Convert.ToString(HttpContext.Current.Session("DateFormat")))
        Else
            CommonFunctions.ShowMessage(COA0016VARIget.ERR, Me.lblFooterMessage)
            Return
        End If
        ...
        COA0016VARIget.COMPCODE = ""
        COA0016VARIget.VARI = Me.hdnThisMapVariant.Value
        COA0016VARIget.FIELD = "ORG"
        COA0016VARIget.COA0016VARIget()
        If COA0016VARIget.ERR = C_MESSAGENO.NORMAL Then
            txtOrg.Text = COA0016VARIget.VALUE
        Else
            CommonFunctions.ShowMessage(COA0016VARIget.ERR, Me.lblFooterMessage)
            Return
        End If
    ...
    Public Sub checkProc()
        ...
        '組織コード
        COA0008InvalidChar.CHARin = txtOrg.Text
        COA0008InvalidChar.COA0008RemoveInvalidChar()
        If COA0008InvalidChar.CHARin = COA0008InvalidChar.CHARout Then
        Else
            txtOrg.Text = COA0008InvalidChar.CHARout
        End If

        '入力項目チェック

        '有効開始日 単項目チェック
        CheckSingle("STYMD", FormatDateYMD(txtStYMD.Text, Convert.ToString(HttpContext.Current.Session("DateFormat"))))
        If returnCode <> C_MESSAGENO.NORMAL Then
            txtStYMD.Focus()
            Return
        End If
        ...
        '有効終了日 単項目チェック
        CheckSingle("ENDYMD", FormatDateYMD(txtEndYMD.Text, Convert.ToString(HttpContext.Current.Session("DateFormat"))))
        If returnCode <> C_MESSAGENO.NORMAL Then
            txtEndYMD.Focus()
            Return
        End If
        ...
        '日付整合性チェック
        CheckDate(FormatDateYMD(txtStYMD.Text, Convert.ToString(HttpContext.Current.Session("DateFormat"))), FormatDateYMD(txtEndYMD.Text, Convert.ToString(HttpContext.Current.Session("DateFormat"))))
        If returnCode <> C_MESSAGENO.NORMAL Then
            txtStYMD.Focus()
            Return
        End If
        ...
        '会社コード 単項目チェック
        CheckSingle("COMPCODE", txtCompany.Text)
        If returnCode <> C_MESSAGENO.NORMAL Then
            txtCompany.Focus()
            Return
        End If
        ...
        '組織コード 単項目チェック
        CheckSingle("ORGCODE", txtOrg.Text)
        If returnCode <> C_MESSAGENO.NORMAL Then
            txtOrg.Focus()
            Return
        End If
    ...
    Protected Sub CheckDate(ByVal inStYMD As String, ByVal inEndYMD As String)
        ...
        If inStYMD <> "" _
            AndAlso inEndYMD <> "" _
            AndAlso wkDateStart > wkDateEnd Then
    ...
    ''' 組織コードリストアイテムを設定
    ''' </summary>
    Private Sub SetOrgListItem(selectedValue As String)
        ...
        Try
            ...
            If Me.lbOrg.Items.Count > 0 Then
                Dim findListItem = Me.lbOrg.Items.FindByValue(selectedValue)
                If findListItem IsNot Nothing Then
                    findListItem.Selected = True
                End If
            End If
    ...
    Public Sub txtOrg_Change()
        ...
        Try
            ...
            If returnCode = C_MESSAGENO.NORMAL AndAlso Me.lbOrg.Items.Count > 0 Then
                Dim findListItem = Me.lbOrg.Items.FindByValue(Me.txtOrg.Text)
                If findListItem IsNot Nothing Then
                    Dim parts As String()
                    parts = Split(findListItem.Text, ":", -1, CompareMethod.Text)
                    Me.lblOrgText.Text = parts(1)
                Else
                    Dim findListItemUpper = Me.lbOrg.Items.FindByValue(Me.txtOrg.Text.ToUpper)
                    If findListItemUpper IsNot Nothing Then
                        Dim parts As String()
                        parts = Split(findListItemUpper.Text, ":", -1, CompareMethod.Text)
                        Me.lblOrgText.Text = parts(1)
                        Me.txtOrg.Text = parts(0)
                    End If
                End If
            End If
