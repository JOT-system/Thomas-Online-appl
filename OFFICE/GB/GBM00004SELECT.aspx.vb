...
''' 顧客検索画面クラス
''' </summary>
Public Class GBM00004SELECT
    Inherits GbPageCommon
    Private Const CONST_MAPID As String = "GBM00004S"       '自身のMAPID
    Private Const CONST_BASEID As String = "GBM00004"
    ...
    Private Sub DisplayLeftView()
        ...
        If changeViewObj IsNot Nothing Then
            ...
            Select Case changeViewObj.ID
                ...
                '顧客コードビュー表示切替
                Case Me.vLeftCustomerCode.ID
                    SetCustomerCodeListItem(Me.txtCustomerCode.Text)
                'カレンダビュー表示切替
                Case Me.vLeftCal.ID
            ...
            Select Case activeViewObj.ID
                ...
                Case Me.vLeftCountryCode.ID 'アクティブなビューが国コード
                    ...
                    If targetObject IsNot Nothing Then
                        ...
                        End If
                    End If
                Case Me.vLeftCustomerCode.ID 'アクティブなビューが顧客コード
                    '顧客コード選択時
                    targetObject = FindControl(Me.hdnTextDbClickField.Value)
                    If targetObject IsNot Nothing Then
                        Dim txtobj As TextBox = DirectCast(targetObject, TextBox)
                        If Me.lbCustomerCode.SelectedItem IsNot Nothing Then
                            txtobj.Text = Me.lbCustomerCode.SelectedItem.Value
                            Dim parts As String()
                            parts = Split(Me.lbCustomerCode.SelectedItem.Text, ":", -1, CompareMethod.Text)
                            Me.lblCustomerCodeText.Text = parts(1)
                            txtobj.Focus()
                        Else
                            txtobj.Text = ""
                            Me.lblCustomerCodeText.Text = ""
                            txtobj.Focus()
                        End If
                    End If
    ...
    Public Sub DefaultValueSet()
        ...
        '選択画面の入力初期値設定
        'メニューから遷移/業務画面戻り判定
        If Page.PreviousPage Is Nothing OrElse TypeOf Page.PreviousPage IsNot GBM00004CUSTOMER Then
        ...
        ElseIf TypeOf Page.PreviousPage Is GBM00004CUSTOMER Then
            Dim prevPage As GBM00004CUSTOMER = DirectCast(Page.PreviousPage, GBM00004CUSTOMER)
            '実行画面からの画面遷移
            '○画面項目設定処理
            ...
            Dim dicObjs As New Dictionary(Of String, TextBox) From {{"hdnSelectedStYMD", Me.txtStYMD},
                                                                    ...
                                                                    {"hdnSelectedCountryCode", Me.txtCountryCode},
                                                                    {"hdnSelectedCustomerCode", Me.txtCustomerCode}
                                                                    }
    ...
    Public Sub variableSet()
        ...
        Dim dicDefaultValueSettings As New Dictionary(Of String, TextBox) _
                        From {{"STYMD", Me.txtStYMD}, {"ENDYMD", Me.txtEndYMD},
                              ...
                              {"CUSTOMERCODE", Me.txtCustomerCode}
                              }
    ...
    Public Sub checkProc()
        ...
        '国コード
        COA0008InvalidChar.CHARin = txtCountryCode.Text
        COA0008InvalidChar.COA0008RemoveInvalidChar()
        If COA0008InvalidChar.CHARin = COA0008InvalidChar.CHARout Then
        Else
            txtCountryCode.Text = COA0008InvalidChar.CHARout
        End If

        '顧客コード
        COA0008InvalidChar.CHARin = txtCustomerCode.Text
        COA0008InvalidChar.COA0008RemoveInvalidChar()
        If COA0008InvalidChar.CHARin = COA0008InvalidChar.CHARout Then
        Else
            txtCustomerCode.Text = COA0008InvalidChar.CHARout
        End If
        ...
        '国コード
        CheckSingle("COUNTRYCODE", txtCountryCode.Text)
        If returnCode <> C_MESSAGENO.NORMAL Then
        ...
        '顧客コード
        CheckSingle("CUSTOMERCODE", txtCustomerCode.Text)
        If returnCode <> C_MESSAGENO.NORMAL Then
        ...
        '②存在チェック(LeftBoxチェック)
        '国コード
        CheckList(txtCountryCode.Text, lbCountryCode)
        If returnCode <> C_MESSAGENO.NORMAL Then
            txtCountryCode.Focus()
            Return
        End If
        '顧客コード
        CheckList(txtCustomerCode.Text, lbCustomerCode)
        If returnCode <> C_MESSAGENO.NORMAL Then
            txtCustomerCode.Focus()
            Return
        End If
    ...
    ''' 顧客コードリストアイテムを設定
    ''' </summary>
    Private Sub SetCustomerCodeListItem(selectedValue As String)
        Dim GBA00004CountryRelated As New GBA00004CountryRelated

        Try

            'リストクリア
            Me.lbCustomerCode.Items.Clear()

            If Me.txtCountryCode.Text <> "" Then
                GBA00004CountryRelated.COUNTRYCODE = Me.txtCountryCode.Text
            End If
            GBA00004CountryRelated.LISTBOX_CUSTOMER = Me.lbCustomerCode
            GBA00004CountryRelated.GBA00004getLeftListCustomer()
            If GBA00004CountryRelated.ERR = C_MESSAGENO.NORMAL OrElse GBA00004CountryRelated.ERR = C_MESSAGENO.NODATA Then
                Me.lbCustomerCode = DirectCast(GBA00004CountryRelated.LISTBOX_CUSTOMER, ListBox)
            Else
            ...
            '一応現在入力しているテキストと一致するものを選択状態
            If Me.lbCustomerCode.Items.Count > 0 Then
                Dim findListItem = Me.lbCustomerCode.Items.FindByValue(selectedValue)
                If findListItem IsNot Nothing Then
                    findListItem.Selected = True
                End If
            End If
    ...
    ''' 顧客コード名設定
    ''' </summary>
    Public Sub txtCustomerCode_Change()

        Try
            Me.lblCustomerCodeText.Text = ""

            SetCustomerCodeListItem(Me.txtCustomerCode.Text)
            If returnCode = C_MESSAGENO.NORMAL AndAlso Me.lbCustomerCode.Items.Count > 0 Then
                Dim findListItem = Me.lbCustomerCode.Items.FindByValue(Me.txtCustomerCode.Text)
                If findListItem IsNot Nothing Then
                    Dim parts As String()
                    parts = Split(findListItem.Text, ":", -1, CompareMethod.Text)
                    Me.lblCustomerCodeText.Text = parts(1)
                Else
                    Dim findListItemUpper = Me.lbCustomerCode.Items.FindByValue(Me.txtCustomerCode.Text.ToUpper)
                    If findListItemUpper IsNot Nothing Then
                        Dim parts As String()
                        parts = Split(findListItemUpper.Text, ":", -1, CompareMethod.Text)
                        Me.lblCustomerCodeText.Text = parts(1)
                        Me.txtCustomerCode.Text = parts(0)
                    End If
                End If
            End If
