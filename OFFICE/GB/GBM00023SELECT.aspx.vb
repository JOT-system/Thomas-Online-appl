...
''' 輸送パターン検索画面クラス
''' </summary>
Public Class GBM00023SELECT
    Inherits GbPageCommon
    Private Const CONST_MAPID As String = "GBM00023S"       '自身のMAPID
    Private Const CONST_BASEID As String = "GBM00023"
    ...
    Private Sub DisplayLeftView()
        ...
        If changeViewObj IsNot Nothing Then
            ...
            Select Case changeViewObj.ID
                ...
                '輸送パターンビュー表示切替
                Case Me.vLeftTransportPattern.ID
                    SetTransportPatternListItem(Me.txtTransportPattern.Text)
                'カレンダビュー表示切替
                Case Me.vLeftCal.ID
    ...
    Public Sub btnEnter_Click()
        ...
        Me.hdnBreakerTypeValue.Value = Me.rblBreakerType.SelectedValue
        Me.hdnBreakerTypeItem.Value = Me.rblBreakerType.SelectedItem.ToString
    ...
    Public Sub btnLeftBoxButtonSel_Click()
        ...
        If activeViewObj IsNot Nothing Then
            Select Case activeViewObj.ID
                'ビューごとの処理はケースを追加で実現
                Case Me.vLeftTransportPattern.ID 'アクティブなビューが輸送パターン
                    '輸送パターン選択時
                    targetObject = FindControl(Me.hdnTextDbClickField.Value)
                    If targetObject IsNot Nothing Then
                        Dim txtobj As TextBox = DirectCast(targetObject, TextBox)
                        If Me.lbTransportPattern.SelectedItem IsNot Nothing Then
                            txtobj.Text = Me.lbTransportPattern.SelectedItem.Value
                            Dim parts As String()
                            parts = Split(Me.lbTransportPattern.SelectedItem.Text, ":", -1, CompareMethod.Text)
                            Me.lblTransportPatternText.Text = parts(1)
                            txtobj.Focus()
                        Else
                            txtobj.Text = ""
                            Me.lblTransportPatternText.Text = ""
                            txtobj.Focus()
                        End If
                    End If
    ...
    Private Sub LangSetting(ByVal lang As String)
        ...
        AddLangSetting(dicDisplayText, Me.lblTransportPattern, "輸送パターン", "Transport Pattern")
    ...
    Public Sub DefaultValueSet()
        ...
        '選択画面の入力初期値設定
        'メニューから遷移/業務画面戻り判定
        If Page.PreviousPage Is Nothing OrElse TypeOf Page.PreviousPage IsNot GBM00023TRPATTERN Then
        ...
        ElseIf TypeOf Page.PreviousPage Is GBM00023TRPATTERN Then
            Dim prevPage As GBM00023TRPATTERN = DirectCast(Page.PreviousPage, GBM00023TRPATTERN)
            '実行画面からの画面遷移
            '○画面項目設定処理
            ...
            Dim dicObjs As New Dictionary(Of String, TextBox) From {{"hdnSelectedStYMD", Me.txtStYMD},
                                                                    ...
                                                                    {"hdnSelectedTransportPattern", Me.txtTransportPattern}
                                                                    }
            ...
            ' ブレーカータイプ
            Me.rblBreakerType.SelectedValue = DirectCast(prevPage.FindControl("hdnSelectedBreakerTypeValue"), HiddenField).Value

            prevViewID = DirectCast(prevPage.FindControl("hdnViewId"), HiddenField).Value

        End If
        'コードを元に名称を設定
        txtTransportPattern_Change()
    ...
    Public Sub checkProc()
        ...
        ''②存在チェック(LeftBoxチェック)
        '輸送パターン
        CheckList(txtTransportPattern.Text, lbTransportPattern)
        If returnCode <> C_MESSAGENO.NORMAL Then
            txtTransportPattern.Focus()
            Return
        End If
    ...
    ''' 輸送パターンリストアイテムを設定
    ''' </summary>
    Private Sub SetTransportPatternListItem(selectedValue As String)

        Try

            'リストクリア
            Me.lbTransportPattern.Items.Clear()
            '検索SQL文
            ...
            sqlStat.AppendLine("  and   tp.BRTYPE = @BRTYPE ")
            ...
            Using SQLcon As New SqlConnection(COA0019Session.DBcon),
                ...
                With SQLcmd.Parameters
                    ...
                    .Add("@BRTYPE", System.Data.SqlDbType.NVarChar).Value = Me.rblBreakerType.SelectedItem.ToString
                End With
            ...
            If Me.lbTransportPattern.Items.Count > 0 Then
                Dim findListItem = Me.lbTransportPattern.Items.FindByValue(selectedValue)
    ...
    ''' 輸送パターン名設定
    ''' </summary>
    Public Sub txtTransportPattern_Change()

        Try
            Me.lblTransportPatternText.Text = ""

            SetTransportPatternListItem(Me.txtTransportPattern.Text)
            If returnCode = C_MESSAGENO.NORMAL AndAlso Me.lbTransportPattern.Items.Count > 0 Then
                Dim findListItem = Me.lbTransportPattern.Items.FindByValue(Me.txtTransportPattern.Text)
                If findListItem IsNot Nothing Then
                    Dim parts As String()
                    parts = Split(findListItem.Text, ":", -1, CompareMethod.Text)
                    Me.lblTransportPatternText.Text = parts(1)
                Else
                    Dim findListItemUpper = Me.lbTransportPattern.Items.FindByValue(Me.txtTransportPattern.Text.ToUpper)
                    If findListItemUpper IsNot Nothing Then
                        Dim parts As String()
                        parts = Split(findListItemUpper.Text, ":", -1, CompareMethod.Text)
                        Me.lblTransportPatternText.Text = parts(1)
                        Me.txtTransportPattern.Text = parts(0)
                    End If
                End If
            End If
