...
Imports BASEDLL
''' <summary>
''' REPAIRBREAKER検索画面クラス
''' </summary>
Public Class GBT00011SELECT
    Inherits GbPageCommon
    Private Const CONST_MAPID As String = "GBT00011S"     '自身のMAPID
    Private Const CONST_BASEID As String = "GBT00011"
        ...
        If changeViewObj IsNot Nothing Then
            ...
            Select Case changeViewObj.ID
                ...
                Case Me.vLeftDepot.ID
                    SetDepotListItem(Me.txtDepot.Text)
                'カレンダビュー表示切替
                Case Me.vLeftCal.ID
            ...
            Select Case activeViewObj.ID
                ...
                Case Me.vLeftTank.ID 'アクティブなビューがタンク番号
                    ...
                    If targetObject IsNot Nothing Then
                        ...
                        End If
                    End If
                Case Me.vLeftDepot.ID 'アクティブなビューがデポ
                    'デポ選択時
                    targetObject = FindControl(Me.hdnTextDbClickField.Value)
                    If targetObject IsNot Nothing Then
                        Dim txtobj As TextBox = DirectCast(targetObject, TextBox)
    ...
    Private Sub LangSetting(ByVal lang As String)
        ...
        AddLangSetting(dicDisplayText, Me.lblYMD1, "作成日", "Created Date")
        AddLangSetting(dicDisplayText, Me.lblYMD2, "範囲指定", "From")
        AddLangSetting(dicDisplayText, Me.lblTilde, "～", "To")
        AddLangSetting(dicDisplayText, Me.lblDepot, "デポ", "Depot")
    ...
    Public Sub DefaultValueSet()
        ...
        '選択画面の入力初期値設定
        'メニューから遷移/業務画面戻り判定
        If Page.PreviousPage Is Nothing OrElse TypeOf Page.PreviousPage IsNot GBT00011RESULT Then
    ...
    Public Sub checkProc()
        ...
        'デポ
        COA0008InvalidChar.CHARin = txtDepot.Text
        COA0008InvalidChar.COA0008RemoveInvalidChar()
        If COA0008InvalidChar.CHARin = COA0008InvalidChar.CHARout Then
        Else
            txtDepot.Text = COA0008InvalidChar.CHARout
        End If
        ...
        'デポ 単項目チェック
        CheckSingle("DEPOTCODE", txtDepot.Text)
        If returnCode <> C_MESSAGENO.NORMAL Then
            txtDepot.Focus()
            Return
        End If
        ...
        'デポ List存在チェック
        CheckList(txtDepot.Text, lbDepot)
        If returnCode <> C_MESSAGENO.NORMAL Then
            txtDepot.Focus()
            Return
        End If
    ...
    ''' デポリストアイテムを設定
    ''' </summary>
    Private Sub SetDepotListItem(selectedValue As String)
        ...
        Try
            ...
            GBA00004CountryRelated.COUNTRYCODE = GBA00003UserSetting.COUNTRYCODE
            GBA00004CountryRelated.LISTBOX_DEPOT = Me.lbDepot
            ...
            If GBA00004CountryRelated.ERR = C_MESSAGENO.NORMAL OrElse GBA00004CountryRelated.ERR = C_MESSAGENO.NODATA Then
                Me.lbDepot = DirectCast(GBA00004CountryRelated.LISTBOX_DEPOT, ListBox)
            Else
            ...
            If Me.lbDepot.Items.Count > 0 Then
                Dim findListItem = Me.lbDepot.Items.FindByValue(selectedValue)
    ...
    ''' デポ設定
    ''' </summary>
    Public Sub txtDepot_Change()
        Try
            Me.lblDepotText.Text = ""
            If Me.txtDepot.Text.Trim = "" Then
                Return
            End If
            SetDepotListItem(Me.txtDepot.Text)
            If returnCode = C_MESSAGENO.NORMAL AndAlso Me.lbDepot.Items.Count > 0 Then
                Dim findListItem = Me.lbDepot.Items.FindByValue(Me.txtDepot.Text)
                If findListItem IsNot Nothing Then
                    Dim parts As String()
                    parts = Split(findListItem.Text, ":", -1, CompareMethod.Text)
                    Me.lblDepotText.Text = parts(1)
                Else
                    Dim findListItemUpper = Me.lbDepot.Items.FindByValue(Me.txtDepot.Text.ToUpper)
                    If findListItemUpper IsNot Nothing Then
                        Dim parts As String()
                        parts = Split(findListItemUpper.Text, ":", -1, CompareMethod.Text)
                        Me.lblDepotText.Text = parts(1)
                        Me.txtDepot.Text = parts(0)
                    End If
                End If
            End If
