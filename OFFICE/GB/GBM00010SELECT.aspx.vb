...
''' 費用項目検索画面クラス
''' </summary>
Public Class GBM00010SELECT
    Inherits GbPageCommon
    Private Const CONST_MAPID As String = "GBM00010S"       '自身のMAPID
    Private Const CONST_BASEID As String = "GBM00010"
        ...
        If changeViewObj IsNot Nothing Then
            ...
            Select Case changeViewObj.ID
                ...
                Case Me.vLeftCostCode.ID
                    SetCostCodeListItem(Me.txtCostCode.Text)
                'カレンダビュー表示切替
                Case Me.vLeftCal.ID
    ...
    Public Sub btnLeftBoxButtonSel_Click()
        ...
        If activeViewObj IsNot Nothing Then
            Select Case activeViewObj.ID
                'ビューごとの処理はケースを追加で実現
                Case Me.vLeftCostCode.ID 'アクティブなビューが費用コード
                    '費用コード選択時
                    targetObject = FindControl(Me.hdnTextDbClickField.Value)
                    If targetObject IsNot Nothing Then
                        Dim txtobj As TextBox = DirectCast(targetObject, TextBox)
                        If Me.lbCostCode.SelectedItem IsNot Nothing Then
                            txtobj.Text = Me.lbCostCode.SelectedItem.Value
                            Dim parts As String()
                            parts = Split(Me.lbCostCode.SelectedItem.Text, ":", -1, CompareMethod.Text)
                            Me.lblCostCodeText.Text = parts(1)
                            txtobj.Focus()
                        Else
                            txtobj.Text = ""
                            Me.lblCostCodeText.Text = ""
                            txtobj.Focus()
                        End If
                    End If
    ...
    Public Sub DefaultValueSet()
        ...
        '選択画面の入力初期値設定
        'メニューから遷移/業務画面戻り判定
        If Page.PreviousPage Is Nothing OrElse TypeOf Page.PreviousPage IsNot GBM00010CHARGECODE Then
        ...
        ElseIf TypeOf Page.PreviousPage Is GBM00010CHARGECODE Then
            Dim prevPage As GBM00010CHARGECODE = DirectCast(Page.PreviousPage, GBM00010CHARGECODE)
            '実行画面からの画面遷移
            '○画面項目設定処理
            ...
            Dim dicObjs As New Dictionary(Of String, TextBox) From {{"hdnSelectedStYMD", Me.txtStYMD},
                                                                    ...
                                                                    {"hdnSelectedCostCode", Me.txtCostCode}
                                                                    }
    ...
    Public Sub checkProc()
        ...
        '費用コード
        COA0008InvalidChar.CHARin = txtCostCode.Text
        COA0008InvalidChar.COA0008RemoveInvalidChar()
        If COA0008InvalidChar.CHARin = COA0008InvalidChar.CHARout Then
        Else
            txtCostCode.Text = COA0008InvalidChar.CHARout
        End If
        ...
        '費用コード
        CheckSingle("COSTCODE", txtCostCode.Text)
        If returnCode <> C_MESSAGENO.NORMAL Then
            txtCostCode.Focus()
            Return
        End If
        ...
        '②存在チェック(LeftBoxチェック)
        '費用コード
        CheckList(txtCostCode.Text, lbCostCode)
        If returnCode <> C_MESSAGENO.NORMAL Then
            txtCostCode.Focus()
            Return
        End If
    ...
    Private Sub SetCostCodeListItem(selectedValue As String)
        ...
        Try
            ...
            '検索SQL文
            SQLStr =
                 "SELECT COSTCODE, MAX(NAMES) AS NAMES, MAX(NAMESJP) AS NAMESJP " _
               & " FROM  GBM0010_CHARGECODE " _
               & " Where STYMD   <= @P1 " _
               & "   and ENDYMD  >= @P2 " _
               & "   and DELFLG  <> @P3 " _
               & " Group by COSTCODE"
            ...
            While SQLdr.Read
                ...
                If COA0019Session.LANGDISP = C_LANG.JA Then
                    Me.lbCostCode.Items.Add(New ListItem(Convert.ToString(SQLdr("COSTCODE")) & ":" & Convert.ToString(SQLdr("NAMESJP")), Convert.ToString(SQLdr("COSTCODE"))))
                Else
                    Me.lbCostCode.Items.Add(New ListItem(Convert.ToString(SQLdr("COSTCODE")) & ":" & Convert.ToString(SQLdr("NAMES")), Convert.ToString(SQLdr("COSTCODE"))))
                End If
            End While
    ...
    Public Sub txtCostCode_Change()
        ...
        Try
            ...
            If returnCode = C_MESSAGENO.NORMAL AndAlso Me.lbCostCode.Items.Count > 0 Then
                Dim findListItem = Me.lbCostCode.Items.FindByValue(Me.txtCostCode.Text)
                If findListItem IsNot Nothing Then
                    Dim parts As String()
                    parts = Split(findListItem.Text, ":", -1, CompareMethod.Text)
                    Me.lblCostCodeText.Text = parts(1)
                Else
                    Dim findListItemUpper = Me.lbCostCode.Items.FindByValue(Me.txtCostCode.Text.ToUpper)
                    If findListItemUpper IsNot Nothing Then
                        Dim parts As String()
                        parts = Split(findListItemUpper.Text, ":", -1, CompareMethod.Text)
                        Me.lblCostCodeText.Text = parts(1)
                        Me.txtCostCode.Text = parts(0)
                    End If
                End If
            End If
