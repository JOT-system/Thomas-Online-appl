<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="GBT00009SELECT.aspx.vb" Inherits="OFFICE.GBT00009SELECT" %>
...
<head runat="server">
    ...
    <style>
       ...
       #itemTable tr.actualDate td:nth-child(2),
       #itemTable tr.actualDate td:nth-child(4) {
           text-align:right;
       }
       /* 検索条件テーブルの行の高さ */
       #itemTable tr {
       ...
       /* コード検索の名称表示部(IDがlblで始まりTextで終わるラベル） */
...
<%--    <!-- Global site tag (gtag.js) - Google Analytics -->
    ...
    <script type="text/javascript">
        ...
        window.addEventListener('DOMContentLoaded', function () {
            ...
            var viewInvoicedBy = '<%= Me.vLeftInvoicedBy.ClientID %>'
            var viewVenderId = '<%= Me.vLeftVender.ClientID %>';                /* 業者コード */
            var viewAgentSoaId = '<%= Me.vLeftAgentSoa.ClientID %>';            /* SOAデータ種別 */
            var viewOfficeId = '<%= Me.vLeftOffice.ClientID %>';                /* 代理店コード */
            var viewCountryId = '<%= Me.vLeftCountry.ClientID %>';              /* 国コード */
            var viewReportMonthId = '<%= Me.vLeftReportMonth.ClientID %>';          /* 計上月 */
            var dblClickObjects = [['<%= Me.txtOffice.ClientID %>', viewOfficeId],
                                   ['<%= Me.txtInvoicedBy.ClientID %>', viewInvoicedBy],
                                   ['<%= Me.txtVender.ClientID %>',viewVenderId],
                                   ['<%= Me.txtAgentSoa.ClientID %>', viewAgentSoaId],
                                   ['<%= Me.txtCountry.ClientID %>', viewCountryId],
                                   ['<%= Me.txtReportMonth.ClientID %>', viewReportMonthId],
                                   ['<%= Me.txtActualDateStYMD.ClientID %>', viewCalId],
                                   ['<%= Me.txtActualDateEndYMD.ClientID %>', viewCalId]];
             ...
             */ 
            var leftListExtentionTarget = [['<%= Me.lbInvoicedBy.ClientID %>', '3', '1'],
                                           ['<%= Me.lbReportMonth.ClientID %>', '3', '1'],
                                           ['<%= Me.lbVender.ClientID %>', '3', '1'],
                                           ['<%= Me.lbAgentSoa.ClientID %>', '3', '1'],
                                           ['<%= Me.lbOffice.ClientID %>', '3', '1'],
                                           ['<%= Me.lbCountry.ClientID %>', '3', '1']
                                          ];
            addLeftBoxExtention(leftListExtentionTarget);
            /* 画面テキストボックス変更イベントのバインド(変更検知したいテキストボックスIDを指定 */
            var targetOnchangeObjects = ['<%= Me.txtInvoicedBy.ClientID %>',
                                         '<%= Me.txtVender.ClientID %>',
                                         '<%= Me.txtCountry.ClientID %>',
                                         '<%= Me.txtOffice.ClientID %>',
                                         '<%= Me.txtAgentSoa.ClientID %>',
                                         '<%= Me.txtReportMonth.ClientID %>'];
...
<body>
    <%--FormIDは適宜変更ください。 --%>
    <form id="GBT00009S" runat="server">
        <%--ヘッダーボックス --%>
        <div id="divContainer">
            <div id="divTitlebox">
            ...
            <div id="divContensbox">
                ...
                <table id="itemTable">
                    ...
                    <tr>
		                <td colspan="2">
			                <a><asp:Label ID="lblCountry" runat="server"></asp:Label></a>
		                </td>
		                <td>
			                <a><asp:TextBox ID="txtCountry" runat="server"></asp:TextBox></a>
		                </td>
                        <td colspan="2">
			                <a><asp:Label ID="lblCountryText" runat="server" ></asp:Label></a>
		                </td>
                        <td></td>
	                </tr>
                    <%-- 代理店コード(SOAは国単位の為 一旦非表示 ) --%> 
                    <tr>
		                <td colspan="2">
                    ...
                    <%-- 請求先種類(JOTを含むか等の選択) --%> 
                    <tr id="trInvoicedBy" runat="server">
		                <td colspan="2">
			                <a><asp:Label ID="lblInvoicedBy" runat="server"></asp:Label></a>
		                </td>
		                <td>
			                <a><asp:TextBox ID="txtInvoicedBy" runat="server"></asp:TextBox></a>
		                </td>
                        <td colspan="2">
			                <a><asp:Label ID="lblInvoicedByText" runat="server" ></asp:Label></a>
		                </td>
                        <td></td>
	                </tr>
                    <%-- 計上月 --%> 
                    <tr>
		                <td colspan="2">
			                <a><asp:Label ID="lblReportMonth" runat="server"></asp:Label></a>
		                </td>
		                <td>
			                <a><asp:TextBox ID="txtReportMonth" runat="server"></asp:TextBox></a>
		                </td>
                        <td colspan="2">
			                <a><asp:Label ID="lblReportMonthText" runat="server" visible="false"></asp:Label></a>
		                </td>
                        <td></td>
	                </tr>
                    <tr class="actualDate">
		                ...
		                <td>
			                <asp:Label ID="lblActualDateF" runat="server"></asp:Label>
		                </td>
		                <td>
			                <asp:TextBox ID="txtActualDateStYMD" runat="server"></asp:TextBox>
		                </td>
		                <td>
			                <asp:Label ID="lblActualDateT" runat="server"></asp:Label>
		                </td>
		                <td>
			                <asp:TextBox ID="txtActualDateEndYMD" runat="server"></asp:TextBox>
		                </td>
                        <td>
                            &nbsp;
                        </td>
                    ...
                    <tr>
		                <td colspan="2">
			                <a><asp:Label ID="lblVender" runat="server"></asp:Label></a>
		                </td>
		                <td>
			                <a><asp:TextBox ID="txtVender" runat="server"></asp:TextBox></a>
		                </td>
                        <td colspan="2">
			                <a><asp:Label ID="lblVenderText" runat="server" ></asp:Label></a>
		                </td>
                        <td></td>
	                </tr>
                    <%-- 承認コード --%> 
                    <tr>
		                <td colspan="2">
			                <a><asp:Label ID="lblAgentSoa" runat="server"></asp:Label></a>
		                </td>
		                <td>
			                <a><asp:TextBox ID="txtAgentSoa" runat="server"></asp:TextBox></a>
		                </td>
                        <td colspan="2">
			                <a><asp:Label ID="lblAgentSoaText" runat="server" ></asp:Label></a>
		                </td>
                        <td></td>
	                </tr>
                ...
                <div id="divHidden">
                    ...
                    <asp:HiddenField ID="hdnButtonClick" runat="server" Value="" /> <%-- ボタン押下(押下したボタンIDを格納) --%><%-- フィールド変更イベントをサーバー処理させるための定義 --%>
                    ...
                    <asp:HiddenField ID="hdnActiveElementAfterOnChange" runat="server" Value="" /> <%-- 変更後イベント直後のフォーカスオブジェクト --%><%-- 左ボックス用情報 --%>
                    ...
                    <asp:HiddenField ID="hdnLeftboxActiveViewId" runat="server" Value="" /> <%-- 左ボックスのアクティブなビュー --%><%-- 右ボックス用情報 --%>                    
                    <asp:HiddenField ID="hdnRightboxOpen" runat="server" Value="" /> <%-- Rightbox 開閉 --%>
                    <asp:HiddenField ID="hdnPrintURL" runat="server" />             <%-- Textbox Print URL --%><%-- フッターヘルプ関連処理で使用 --%>
            ...
            <div id="divLeftbox">
                ...
                <asp:MultiView ID="mvLeft" runat="server">
                    <%--  　InvoicedBy種類コード　 --%>
                    <asp:View id="vLeftInvoicedBy" runat="server" >
                        <div class="leftViewContents">
                            <asp:ListBox ID="lbInvoicedBy" runat="server" CssClass="leftViewContents"></asp:ListBox>                           
                        </div>
                    </asp:View> <%-- END InvoicedBy種類コード VIEW　 --%>
                    <%--  　SOA種類コード　 --%>
                    <asp:View id="vLeftAgentSoa" runat="server" >
                        <div class="leftViewContents">
                            <asp:ListBox ID="lbAgentSoa" runat="server" CssClass="leftViewContents"></asp:ListBox>                           
                        </div>
                    </asp:View> <%-- END SOA種類コード VIEW　 --%>
                    <%--  　計上月種別コード　 --%>
            ...
            <div id="divRightbox">
                ...
                <div id="divRightMessageBox">
                    ...
                    <div id="divMessageType" runat="server" >
                        ...
                        <%-- エラー詳細のみ表示の場合はrbShowErrorの文言のみ表示 --%><%= IIf(Me.divMessageChooseArea.Visible = False And
