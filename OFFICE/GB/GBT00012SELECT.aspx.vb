...
''' Repair承認検索画面クラス
''' </summary>
Public Class GBT00012SELECT
    Inherits GbPageCommon
    Private Const CONST_MAPID As String = "GBT00012S"       '自身のMAPID
    Private Const CONST_BASEID As String = "GBT00012A"
    ...
    Private Sub DisplayLeftView()
        ...
        If changeViewObj IsNot Nothing Then
            ...
            Select Case changeViewObj.ID
                ...
                'LastCargoビュー表示切替
                Case Me.vLeftLastCargo.ID
                    SetLastCargoListItem(Me.txtLastCargo.Text)
                'Locationビュー表示切替
                Case Me.vLeftLocation.ID
                    SetLocationListItem(Me.txtLocation.Text)
                'カレンダビュー表示切替
                Case Me.vLeftCal.ID
    ...
    Public Sub btnLeftBoxButtonSel_Click()
        ...
        If activeViewObj IsNot Nothing Then
            Select Case activeViewObj.ID
                Case Me.vLeftTank.ID 'アクティブなビューがタンク番号
                    'タンク番号選択時
                ...
                Case Me.vLeftLastCargo.ID
                    ...
                    If targetObject IsNot Nothing Then
                        Dim txtobj As TextBox = DirectCast(targetObject, TextBox)
                        If Me.lbLastCargo.SelectedItem IsNot Nothing Then
                            txtobj.Text = Me.lbLastCargo.SelectedItem.Value
                        Else
                            txtobj.Text = ""
                        End If
                        txtLastCargo_Change()
    ...
    Private Sub LangSetting(ByVal lang As String)
        ...
        AddLangSetting(dicDisplayText, Me.lblLastCargo, "Last Cargo", "Last Cargo")
    ...
    Public Sub DefaultValueSet()
        ...
        '選択画面の入力初期値設定
        'メニューから遷移/ 業務画面戻り判定
        If Page.PreviousPage Is Nothing OrElse TypeOf Page.PreviousPage IsNot GBT00012APPROVAL Then
        ...
        ElseIf TypeOf Page.PreviousPage Is GBT00012APPROVAL Then
            ...
            Dim dicObjs As New Dictionary(Of String, TextBox) From {{"hdnStYMD", Me.txtStYMD},
                                                                    ...
                                                                    {"hdnLastCargo", Me.txtLastCargo},
                                                                    {"hdnLocation", Me.txtLocation}
                                                                    }
    ...
    Public Sub variableSet()
        ...
        Dim dicDefaultValueSettings As New Dictionary(Of String, TextBox) _
                        ...
                        {"TANKNO", Me.txtTankNo}, {"LASTCARGO", Me.txtLastCargo}, {"LOCATION", Me.txtLocation}
                              }
    ...
    Public Sub checkProc()
        ...
        'Last Cargo
        COA0008InvalidChar.CHARin = txtLastCargo.Text
        COA0008InvalidChar.COA0008RemoveInvalidChar()
        If COA0008InvalidChar.CHARin = COA0008InvalidChar.CHARout Then
        Else
            txtLastCargo.Text = COA0008InvalidChar.CHARout
        End If

        'Location
        COA0008InvalidChar.CHARin = txtLocation.Text
        COA0008InvalidChar.COA0008RemoveInvalidChar()
        If COA0008InvalidChar.CHARin = COA0008InvalidChar.CHARout Then
        Else
            txtLocation.Text = COA0008InvalidChar.CHARout
        End If
        ...
        '③相関チェック
        '申請開始日<=申請終了日
        If txtStYMD.Text <> "" And txtEndYMD.Text <> "" Then
            If txtStYMD.Text > txtEndYMD.Text Then
        ...
        '終了日が空の場合、開始日を設定
        If txtEndYMD.Text = "" Then
            txtEndYMD.Text = txtStYMD.Text
        End If
        ...
        'タンク番号 List存在チェック
        CheckList(txtTankNo.Text, lbTank)
        If returnCode <> C_MESSAGENO.NORMAL Then
            txtTankNo.Focus()
            Return
        End If
        ...
        'Last Cargo 単項目チェック
        CheckSingle("LASTCARGO", txtLastCargo.Text)
        If returnCode <> C_MESSAGENO.NORMAL Then
            txtLastCargo.Focus()
            Return
        End If

        'Last Cargo List存在チェック
        CheckList(txtLastCargo.Text, lbLastCargo)
        If returnCode <> C_MESSAGENO.NORMAL Then
            txtLastCargo.Focus()
            Return
        End If
        ...
        'Location 単項目チェック
        CheckSingle("LOCATION", txtLocation.Text)
        If returnCode <> C_MESSAGENO.NORMAL Then
            txtLocation.Focus()
            Return
        End If
        ...
        'Location List存在チェック
        CheckList(txtLocation.Text, lbLocation)
        If returnCode <> C_MESSAGENO.NORMAL Then
            txtLocation.Focus()
            Return
        End If
    ...
    ''' LastCargo設定
    ''' </summary>
    Public Sub txtLastCargo_Change()
        Try
            Me.lblLastCargoText.Text = ""
            If Me.txtLastCargo.Text.Trim = "" Then
                Return
            End If
            SetLastCargoListItem(Me.txtLastCargo.Text)
            If returnCode = C_MESSAGENO.NORMAL AndAlso Me.lbLastCargo.Items.Count > 0 Then
                Dim findListItem = Me.lbLastCargo.Items.FindByValue(Me.txtLastCargo.Text)
                If findListItem IsNot Nothing Then
                    Dim parts As String()
                    parts = Split(findListItem.Text, ":", -1, CompareMethod.Text)
                    Me.lblLastCargoText.Text = parts(1)
                Else
                    Dim findListItemUpper = Me.lbLastCargo.Items.FindByValue(Me.txtLastCargo.Text.ToUpper)
                    If findListItemUpper IsNot Nothing Then
                        Dim parts As String()
                        parts = Split(findListItemUpper.Text, ":", -1, CompareMethod.Text)
                        Me.lblLastCargoText.Text = parts(1)
                        Me.txtLastCargo.Text = parts(0)
                    End If
                End If
            End If
    ...
    ''' タンク番号リストアイテムを設定
    ''' </summary>
    Private Sub SetTankListItem(selectedValue As String)
        ...
        Try
            ...
            Else
                returnCode = GBA00012TankInfo.ERR
                Return
            End If
    ...
    ''' 積載品コードリストアイテムを設定
    ''' </summary>
    Private Sub SetLastCargoListItem(selectedValue As String)
        Try
            Me.lbLastCargo.Items.Clear() 'リストクリア
                ...
                Using SQLdr = SQLcmd.ExecuteReader()
                    While SQLdr.Read
                        ...
                        If COA0019Session.LANGDISP = C_LANG.JA Then
                            Me.lbLastCargo.Items.Add(New ListItem(String.Format("{0}:{1}", SQLdr("PRODUCTCODE"), SQLdr("NAMESJP")), Convert.ToString(SQLdr("PRODUCTCODE"))))
                        Else
                            Me.lbLastCargo.Items.Add(New ListItem(String.Format("{0}:{1}", SQLdr("PRODUCTCODE"), SQLdr("NAMES")), Convert.ToString(SQLdr("PRODUCTCODE"))))
                        End If
            ...
            '一応現在入力しているテキストと一致するものを選択状態
            If Me.lbLastCargo.Items.Count > 0 Then
                Dim findListItem = Me.lbLastCargo.Items.FindByValue(selectedValue)
                If findListItem IsNot Nothing Then
                    findListItem.Selected = True
                End If
    ...
    ''' ロケーションリストアイテムを設定
    ''' </summary>
    Private Sub SetLocationListItem(selectedValue As String)
        Try
            Me.lbLocation.Items.Clear() 'リストクリア

            Dim sqlStat As New StringBuilder
            sqlStat.AppendLine("SELECT LOCATION")
            ...
            sqlStat.AppendLine("GROUP BY LOCATION")
            ...
            Using SQLcon As New SqlConnection(COA0019Session.DBcon),
                ...
                Using SQLdr = SQLcmd.ExecuteReader()
                    While SQLdr.Read
                        Me.lbLocation.Items.Add(New ListItem(String.Format("{0}", SQLdr("LOCATION"))))
                    End While
                End Using
            End Using

            '一応現在入力しているテキストと一致するものを選択状態
            If Me.lbLocation.Items.Count > 0 Then
                Dim findListItem = Me.lbLocation.Items.FindByValue(selectedValue)
                If findListItem IsNot Nothing Then
                    findListItem.Selected = True
                End If
