...
function bindGridButtonClickEvent() {
    ...
    var colCond = "th[cellfiedlname='BLISSUE'],th[cellfiedlname='ODID'],th[cellfiedlname^='ETD'],th[cellfiedlname^='ETA'],th[cellfiedlname='CONSIGNEE'],th[cellfiedlname='NOOFTANKS'],th[cellfiedlname='BOOKINGNO'],th[cellfiedlname='VSL1'],th[cellfiedlname='VOY1']";
    ...
    for (let i = 0; i < buttonList.length; i++) {
        ...
        if (brOdFlgCell.innerText === '1') {
            ...
            /* 削除不能(出航等)した行に対しては削除ボタンを使用不可にする */
            if (rightTrNode.dataset.cannotdelete === '1') {
                buttonObj.disabled = 'true';
                buttonObj.classList.add('aspNetDisabled');
                continue;
            }
            buttonObj.onclick = (function (buttonObj) {
                return function () {
...
// 〇削除モードチェックボックス変更時イベント
function chkShowDeleteChanged() {
    /* 処理に必要なオブジェクトがレンダリングされていない場合はそのまま終了 */
    var chkId = 'chkShowDelete';
    var chkDelModeObj = document.getElementById(chkId);
    if (chkDelModeObj === null) {
        return;
    }

    var listDibObj = document.getElementById('WF_LISTAREA');
    if (listDibObj === null) {
        return;
    }

    listDibObj.classList.remove('hideDelete');
    listDibObj.classList.remove('showDelete');

    var delModeFlg = 'hideDelete';
    if (chkDelModeObj.checked) {
        delModeFlg = 'showDelete';
    }
    listDibObj.classList.add(delModeFlg);
}