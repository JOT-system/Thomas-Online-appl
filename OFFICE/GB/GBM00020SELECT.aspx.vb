...
''' 為替レート検索画面クラス
''' </summary>
Public Class GBM00020SELECT
    Inherits GbPageCommon
    Private Const CONST_MAPID As String = "GBM00020S"       '自身のMAPID
    Private Const CONST_BASEID As String = "GBM00020"
        ...
        If changeViewObj IsNot Nothing Then
            ...
            Select Case changeViewObj.ID
                ...
                Case Me.vLeftCal.ID
                    ...
                    'If Date.TryParse(txtStYMD.Text, stYmd) Then
                    '    Me.hdnCalendarValue.Value = stYmd.ToString("yyyy/MM/01")
                    'Else
                    '    Me.hdnCalendarValue.Value = txtStYMD.Text
                    'End If

                    ' カレンダーは常にFROM基準で表示
                    Me.hdnCalendarValue.Value = FormatDateYMD(txtStYMD.Text, GBA00003UserSetting.DATEYMFORMAT)
    ...
    Private Sub LangSetting(ByVal lang As String)
        ...
        AddLangSetting(dicDisplayText, Me.lblYMD1, "対象年月", "Target Date")
    ...
    Public Sub DefaultValueSet()
        ...
        '選択画面の入力初期値設定
        'メニューから遷移/業務画面戻り判定
        If Page.PreviousPage Is Nothing OrElse TypeOf Page.PreviousPage IsNot GBM00020EXRATE Then
        ...
        ElseIf TypeOf Page.PreviousPage Is GBM00020EXRATE Then
            Dim prevPage As GBM00020EXRATE = DirectCast(Page.PreviousPage, GBM00020EXRATE)
            '実行画面からの画面遷移
            '○画面項目設定処理
            ...
            For Each dicObj As KeyValuePair(Of String, TextBox) In dicObjs
                ...
                If tmpCont IsNot Nothing Then
                    ...
                    If dicObj.Key = "hdnSelectedStYMD" OrElse dicObj.Key = "hdnSelectedEndYMD" Then
                        dicObj.Value.Text = FormatDateContrySettings(DirectCast(tmpCont, HiddenField).Value, GBA00003UserSetting.DATEYMFORMAT)
                    Else
        ...
        For Each item As KeyValuePair(Of String, TextBox) In dicDefaultValueSettings
            ...
            If COA0016VARIget.ERR = C_MESSAGENO.NORMAL Then
                If item.Key = "STYMD" OrElse item.Key = "ENDYMD" Then
                    ...
                    'If Date.TryParse(COA0016VARIget.VALUE, pDate) Then
                    '    item.Value.Text = pDate.ToString("yyyy/MM")
                    'Else
    ...
    Public Sub checkProc()
        ...
        '①単項目チェック
        '有効開始日
        CheckSingle("STYMD", FormatDateYMD(txtStYMD.Text, GBA00003UserSetting.DATEYMFORMAT))
        If returnCode <> C_MESSAGENO.NORMAL Then
            txtStYMD.Focus()
            Return
        End If
        ...
        '有効終了日
        CheckSingle("ENDYMD", FormatDateYMD(txtEndYMD.Text, GBA00003UserSetting.DATEYMFORMAT))
        If returnCode <> C_MESSAGENO.NORMAL Then
            txtEndYMD.Focus()
            Return
        End If
        ...
        If txtStYMD.Text <> "" AndAlso txtEndYMD.Text <> "" Then
            ...
            '    Return
            'End If
            CheckDate(FormatDateYMD(txtStYMD.Text, GBA00003UserSetting.DATEYMFORMAT), FormatDateYMD(txtEndYMD.Text, GBA00003UserSetting.DATEYMFORMAT))
            If returnCode <> C_MESSAGENO.NORMAL Then
                txtStYMD.Focus()
                Return
            End If
        ...
        '未来日付チェック
        If txtStYMD.Text <> "" Then
            If Date.Parse(txtStYMD.Text) > Date.Parse(Date.Now.AddMonths(1).ToString("yyyy/MM")) Then
                returnCode = C_MESSAGENO.VALIDITYINPUT
                CommonFunctions.ShowMessage(returnCode, Me.lblFooterMessage, pageObject:=Me)
                txtStYMD.Focus()
                Return
        ...
        If txtEndYMD.Text <> "" Then
            If Date.Parse(txtEndYMD.Text) > Date.Parse(Date.Now.AddMonths(1).ToString("yyyy/MM")) Then
                returnCode = C_MESSAGENO.VALIDITYINPUT
                CommonFunctions.ShowMessage(returnCode, Me.lblFooterMessage, pageObject:=Me)
                txtStYMD.Focus()
                Return
